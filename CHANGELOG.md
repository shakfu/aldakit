...
- **`tests/test_invariants.py`, properties rather than fixtures.** A fixture records what the implementation currently does; a property records what `docs/reference.md` promises. Strip every barline and the MIDI must be identical, because barlines are documented as purely visual -- likewise comments, likewise runs of whitespace. The barline property is that sentence of the reference made executable, and it fails on three corpus examples against the old parser with no fixture involved. Grow this file whenever the reference claims that something has no musical effect, or that two spellings mean the same thing.

- **The language reference documents ties across barlines.** `docs/reference.md` now shows the three spellings -- `c4~|2`, `c4 | ~2`, `c4~|~2` -- and states that all of them mean what `c4~2` means.

- **`make fix`**, the mutating half of what `make lint` used to do.

### Changed

- **`repl.py` and `libremidi_backend.py` are typechecked.** Both were excluded from `ty` outright -- 583 statements, including the REPL, the largest module after `cli.py`, so "typecheck is clean" covered less than it appeared to. Three things were in the way, and each wanted a different answer. The eight unresolved `prompt_toolkit` imports are a consequence of the vendoring: the copy reaches the module path at runtime through `ext/__init__.py`, which a type checker has no way to know, so `[tool.ty.environment] extra-paths` now points at it. The seven unresolved attributes were real: `ReplContext.backend` was typed `object`, so nothing about it could be checked at all. It is now a `ReplBackend` protocol, with the members only the MIDI backend has -- `close`, `list_output_ports`, `active_slots`, `concurrent_mode` -- split into a `ConcurrentBackend` protocol that the guarded commands read through a single `_concurrent` helper. Protocols rather than a base class because command handling is exercised with a structural stand-in. The last one, the compiled `_libremidi` module, has no stubs to check against and takes the `# type: ignore[attr-defined]` the other native import already used.

- **`make qa` runs the tests and no longer rewrites source.** It was `lint typecheck format`, where `lint` was `ruff check --fix` and `format` rewrote files: a quality-assurance target that edited the working tree and never ran the suite. It is now `lint typecheck test`, and `make lint` is read-only and identical to what CI runs. `make fix` and `make format` are the ones that edit.

- **Publishing waits for the golden audio comparison.** The `audio` job had no dependents, so a red audio comparison blocked neither the wheel build nor the upload to PyPI: the fixture discipline this project relies on was not wired into its release path. `publish` and `prerelease` now require it. `build_wheels` still depends only on `test`, so pull request feedback is unchanged and wheels remain available as artifacts when audio is red.

### Removed

- **`pytest-review` and the `make test-review` target.** It scored the suite 0/100 (F) on every run, and was wired into neither CI nor `make qa`. Reading the findings rather than the score settles what it was worth: all 8 errors are false positives on deliberate smoke tests -- `score.stop()  # must not raise` has no assertion because not raising *is* the assertion, and `test_equivalent_to_source` delegates its assertion to a helper -- and the 689 warnings are almost entirely "assertion without a message". Adding those messages would have bought a number and no correctness, and a gate reporting F on every run trains people to ignore gates. Removed rather than left unenforced.

### Fixed

- **A tie no longer loses its duration at a barline.** `docs/reference.md` says barlines are purely visual and have no effect on timing; the parser ended a tie chain at one and silently discarded the duration that followed. Affected were `debussy_quartet`, `gau`, `jimenez-divertimento` and `rachmaninoff_piano_concerto_2_mvmt_2`, in every case a note held across a bar coming out clipped and everything after it starting early. A tie now steps over barlines and line breaks and may be written on either side of the bar or on both, which is how all three forms occur in those scores: `a-8~|2.`, `g+2 | ~2`, and `d4.~4~|` continued by `|~4.~8` on the next line. The barline is recorded on the duration node so `AldaWriter` writes it back.

- **The parser reports a token it cannot place instead of dropping it.** Anything `_parse_primary_event` could not consume was skipped without an error, so `piano: c d ]` parsed clean and `aldakit lint` reported no problems. That silence is how the tie bug stayed invisible: the token being discarded *was* the music. An unconsumable token is now a syntax error carrying a hint that names what it should have attached to -- an unbalanced `]`, a duration with no note in front of it, a repeat with nothing to repeat. **This can reject a score that was previously accepted.** It was never accepted in the sense of being understood: the tokens now being named are exactly the ones that used to be thrown away, so a score that newly fails is one that was already not producing the music it described.

- **The native extensions no longer build into the source tree.** `src/CMakeLists.txt` set `RUNTIME_OUTPUT_DIRECTORY_RELEASE` and its siblings to `src/aldakit`, so building for one Python version left that version's `.so` sitting in the source directory while another was being built -- untracked, ignored by git, and a familiar way to get "works locally, fails in CI". The `install(TARGETS ...)` directives are what put the extensions in the wheel and are unaffected; a clean rebuild now leaves `src/aldakit` empty of artifacts, and the wheel still carries both.

- **Removed a dead Homebrew probe from `CMakeLists.txt`.** It ran `brew --prefix` on macOS hosts to set `local_prefix`, which was then read by nothing but a `message(STATUS)`. Its guard was broken too -- `execute_process` always defines the output variable, so `if(DEFINED homebrew_prefix AND ...)` turned entirely on the other operand.
